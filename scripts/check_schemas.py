...
def main() -> None:
    ...
    for entry in entries:
        ...
        if not isinstance(schema_url, str):
            fail(f"missing schema URL for {schema_id}")
        ...
        if not isinstance(instance_url, str):
            fail(f"invalid instance URL for {schema_id}")
    ...
    model_response = json.loads((ROOT / "schemas" / "agent-model-response-v1.schema.json").read_text(encoding="utf-8"))
    required_model_response = set(model_response.get("required", []))
    for key in ("case_id", "answer", "route", "resource_ids", "source_record_ids", "locale_behavior"):
        if key not in required_model_response:
            fail(f"agent model response schema must require {key}")

    model_run = json.loads((ROOT / "schemas" / "agent-model-run-v1.schema.json").read_text(encoding="utf-8"))
    required_model_run = set(model_run.get("required", []))
    for key in (
        "id", "eval_dataset", "eval_dataset_sha256", "executed", "model", "prompt",
        "evaluator", "metrics", "case_results", "semantic_evaluation", "notes",
    ):
        if key not in required_model_run:
            fail(f"agent model run schema must require {key}")
    model_required = set(model_run["properties"]["model"].get("required", []))
    for key in ("provider", "model", "version_or_snapshot", "runtime", "runtime_metadata"):
        if key not in model_required:
            fail(f"agent model run provenance must require {key}")
    prompt_props = model_run["properties"]["prompt"]["properties"]
    if prompt_props.get("expected_fields_hidden", {}).get("const") is not True:
        fail("agent model run schema must require expected_fields_hidden=true")
    if prompt_props.get("url", {}).get("const") != f"{ORIGIN}/research/model-run-prompt-v1.txt":
        fail("agent model run schema must pin model-run-prompt-v1 URL")
    evaluator_props = model_run["properties"]["evaluator"]["properties"]
    if evaluator_props.get("id", {}).get("const") != "route-contract-evaluator-v1":
        fail("agent model run schema must identify deterministic route evaluator v1")
    semantic_props = model_run["properties"]["semantic_evaluation"]["properties"]
    for key in ("automatic_expected_checks_scored", "automatic_prohibited_claims_scored"):
        if semantic_props.get(key, {}).get("type") != "boolean":
            fail(f"agent model run schema must expose {key}")

    print(
        f"schema check passed: {len(entries)} versioned contracts; "
        f"{interface_only} interface-only contracts awaiting published instances"
    )
