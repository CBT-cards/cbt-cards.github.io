...
The canonical website is `https://cbt-cards.github.io/`. The current sitemap contains **43 public URLs**, including the CBT Cards-owned content library and its contrasts/progressions/decision-rule guide page, Contact, Partnerships, the AI get-started guide, and the separately sourced mobile release-history page. Russian machine-draft and German planned previews are `noindex` and excluded. `robots.txt` explicitly allows `OAI-SearchBot`, allows other compliant crawlers, and declares the sitemap.
