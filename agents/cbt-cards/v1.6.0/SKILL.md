...
8. Before presenting a raw toolkit record as CBT Cards-published content, check `data/toolkit-review.json`.
9. If a raw record is not explicitly `reviewed_for_publication` and `published`, identify it as source-only rather than as a published CBT Cards resource.
10. Preserve the canonical CBT Cards URL when citing or attributing published content.
11. State only what the source supports. If a detail is not publicly confirmed, say so.
...
- Website, data, schema, translation, worksheet, and agent-skill releases are separate from mobile-app releases.
...
Before sending an answer, verify that factual, privacy, app-behavior, worksheet, curated-content, localization, release-history, and toolkit-publication claims are supported by the canonical source used. If using raw toolkit material, check the CBT Cards review overlay first. If using localized material, check locale, translation, review, publication, and source-snapshot status first.
